...
`2026` 向けの更新後のコードは完成サンプルなので、起動するとレシピ一覧が表示されます。Schemeの `USE_STUB_API_CLIENT` は初期値が `1` で、外部サーバーなしで取得・タグ追加・再取得を試せます。追加したタグはアプリ終了で消えます。以下の「Hello, world!」と画像は2023年版の開始時点のものです。

実APIを使う場合はこの環境変数を `0` にします。リクエスト先は `https://localhost:3001` と `https://localhost:3002` で、対応サーバーと証明書の準備はこのiOS教材には含まれません。
