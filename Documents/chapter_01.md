...
## [補足] 2026 年版のサンプルを起動する場合

2026 年版の完成サンプルでは、起動時にレシピ一覧が表示されます。Scheme の環境変数`USE_STUB_API_CLIENT`が`1`になっているので、外部サーバーを起動せずにレシピの取得とハッシュタグの追加を試すことができます。追加したハッシュタグはアプリを終了すると消えます。以下の「Hello, world!」の画面例は、2023 年版の実習開始時点のものです。

実際の API に接続する場合は、この環境変数を`0`に変更してください。接続先は`https://localhost:3001`と`https://localhost:3002`です。サーバーと証明書の用意については、今回の教材では扱いません。
