...
1. Previewのクライアントを `StubAPIClient(scenario: .empty)` に変えて空状態を見る。
2. `.failure` に変えて失敗・再試行ボタンを見る。常に失敗する設定なので、成功への遷移はChapter 7のテストで確認する。
3. `delay: .seconds(5)` で読み込み中を確認する。
4. `hasLoaded` がないと、起動直後と取得成功後の空一覧を区別しにくい理由を説明する。
